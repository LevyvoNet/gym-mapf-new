...
tsl::hopscotch_set<Location> *get_reachable_locations(CrossedPolicy *joint_policy,
                                                      size_t agent, double timeout_ms) {
    MEASURE_TIME;
    ...
    *expanded = false;
    ...
    do {
        ...
        *expanded = true;
        ...
        action = policy->act(curr_state,timeout_ms -  ELAPSED_TIME_MS);
...
bool are_sharing_states(CrossedPolicy *joint_policy, size_t agent1, size_t agent2, double timeout_ms) {
    MEASURE_TIME;
    tsl::hopscotch_set<Location> *locs1 = get_reachable_locations(joint_policy, agent1, timeout_ms - ELAPSED_TIME_MS);
    tsl::hopscotch_set<Location> *locs2 = get_reachable_locations(joint_policy, agent2, timeout_ms - ELAPSED_TIME_MS);
...
Conflict *detect_conflict(CrossedPolicy *joint_policy, double timeout_ms) {
    ...
    for (size_t agent1 = 0; agent1 < joint_policy->env->n_agents; ++agent1) {
        for (size_t agent2 = 0; agent2 < joint_policy->groups.size(); ++agent2) {
            if (agent1 < agent2 && are_sharing_states(joint_policy, agent1, agent2, timeout_ms - ELAPSED_TIME_MS)) {
